...
void CMydemoView::OnDraw(CDC* pDC)
{
	...
	for (i = 1; i < 6; i++)
		for (j = 1; j < 6; j++)
		{
			if (pDoc->T1[i][j] > pDoc->endtemp)
				color = int(255 * fabs(pDoc->T1[i][j] - pDoc->endtemp) / (1370 - pDoc->endtemp));
			else
				color = 0;
			...
			pDC->SetViewportOrg(100, 100 + (j - 1) * eagelenth);
